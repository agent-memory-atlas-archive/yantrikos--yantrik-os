...
| `companion_tool(name, args)` | run one of the mind's own tools by name over `companion.sock`, with no language model in the loop — so what is measured is the tool and not a model's willingness to choose it. Always a dict: `text` is what the tool said, refusals included, because these tools answer prose; `error` is the transport or the registry — no such tool, the worker gone, no socket on this machine. Check `error` first, or "the shell is not running" reads as "the tool declined". |
