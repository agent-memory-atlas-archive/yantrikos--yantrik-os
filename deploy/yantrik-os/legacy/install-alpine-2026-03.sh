#!/bin/bash
# ═══════════════════════════════════════════════════════════════════════════════════════
# NOT USED. This is the March 2026 installer, kept as a record. Do not run it.
# ═══════════════════════════════════════════════════════════════════════════════════════
#
# This was `install.sh` at the root of the repository until September 2026, and it is what
# https://get.yantrikos.com/install.sh serves at the time this header was written. It
# installs a set of binaries onto an already-running **Alpine Linux** machine, from
# `get.yantrik.dev` — a domain that no longer resolves — and sets up `yantrik-upgrade`, a
# program that no longer exists.
#
# Every one of those is now wrong. Yantrik OS is a Debian 13 live ISO built by CI and
# published to https://iso.yantrikos.com/nightly/; you boot the image and run
# `yantrik-install` inside it; an installed machine updates itself with `yantrik-update`.
# Running this script against anything gets you a different operating system with an
# unrelated set of files on it.
#
# It is kept rather than deleted for one reason: until the server is redeployed, this is the
# script strangers are being handed, and the repository should contain the thing being
# served. The replacement is `install.sh` at the root — it downloads and verifies an ISO and
# installs nothing.
#
# It stays executable only because CI requires every tracked file starting with `#!` to be.
